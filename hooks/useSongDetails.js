...
const getBestImage = (images) => {
  ...
  return bestImage?.url || bestImage?.link || null;
};
    ...
    const fetchSongDetails = async () => {
      ...
      try {
        ...
        if (isYTMusicTrack) {
          ...
          setSongDetails({
            basicInfo: [
              {
                label: 'Title',
                value: track.title || track.name || 'Unknown Track',
              },
        ...
        if (isSpotifyTrack) {
          ...
          setSongDetails({
            ...
            additionalInfo: [
              {
                label: 'Spotify ID',
                value: track.id || track.spotifyId || 'N/A',
              },
              { label: 'Quality', value: actualQuality },
              { label: 'Explicit', value: track.explicit ? 'Yes' : 'No' },
              {
                label: 'Preview',
                value: track.previewUrl ? 'Available' : 'N/A',
              },
            ],
            mediaInfo: [
              ...
              {
                label: 'YT Content ID',
                value: track.ytMusicVideoId || 'Mapped on Play',
              },
            ],
            imageUrl: track.artwork || track.image,
        ...
        const isDabTrack =
          ...
          (track.url &&
            (track.url.includes('qobuz') || track.url.includes('dab-music'))) ||
          (!isNaN(track.url) &&
            String(track.url).length > 5 &&
            String(track.url).length < 15) ||
          (!isNaN(track.id) &&
            String(track.id).length >= 6 &&
            String(track.id).length <= 7); // Reliable DAB ID detection
        ...
        if (response.data && response.data.success) {
          ...
          const primaryArtists =
            data.primaryArtists || formatArtists(data.artists?.primary);
          const featuredArtists =
            data.featuredArtists || formatArtists(data.artists?.featured);
          const allArtists =
            data.primaryArtists || formatArtists(data.artists?.all);
          ...
          const basicInfo = [
            {
              label: 'Title',
              value: data.name || track.title || 'Unknown Track',
            },
            { label: 'Artists', value: primaryArtists },
            {
              label: 'Album',
              value: data.album?.name || track.album || 'Unknown Album',
            },
            {
              label: 'Duration',
