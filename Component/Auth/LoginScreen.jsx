...
const sha1 = (message) => {
    ...
    if (i === 0) {
        ...
        temp = (msg.charCodeAt(msgLen - 2) << 24) | (msg.charCodeAt(msgLen - 1) << 16) | 0x8000;
        wordArray.push(temp);
    } else {
        temp = (msg.charCodeAt(msgLen - 3) << 24) | (msg.charCodeAt(msgLen - 2) << 16) | (msg.charCodeAt(msgLen - 1) << 8) | 0x80;
        wordArray.push(temp);
    }
...
const fetchYouTubeAccountInfo = async (cookieString) => {
    try {
        ...
        const response = await fetch('https://music.youtube.com/youtubei/v1/account/account_menu?prettyPrint=false', {
            method: 'POST',
            headers: {
...
`);
    ...
    const saveAndSendCookies = async (cookieString, { fromAutoCapture = false, userInfo = null } = {}) => {
        try {
                        ...
                        onMessage={(event) => {
                            ...
                            try {
                                ...
                                if (parsed.type === 'userInfo' && parsed.data) {
                                    ...
                                    const hasGoodName = currentUser?.name && currentUser.name !== 'YouTube User';
                                    const newHasName = parsed.data.name && parsed.data.name !== '' && parsed.data.name !== 'YouTube User';

                                    if (newHasName || !hasGoodName) {
                                        // Merge data, keeping existing good values
                                        ytAuthService.setUser({
                                            name: (parsed.data.name && parsed.data.name !== '') ? parsed.data.name : (currentUser?.name || 'YouTube User'),
                                            handle: parsed.data.handle || currentUser?.handle || '',
                                            avatarUrl: parsed.data.avatarUrl || currentUser?.avatarUrl || null
                                        });
