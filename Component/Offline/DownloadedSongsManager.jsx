...
const DownloadedSongsManager = ({
  ...
  const loadDownloadedSongs = useCallback(async () => {
    try {
      ...
      for (const [songId, metadata] of Object.entries(allMetadata)) {
        try {
          ...
          if (songExists) {
            ...
            console.warn(
              `DownloadedSongsManager: Song file not found for ${songId}, will be cleaned up`
            );
          }
        } catch (songError) {
          console.error(
            `DownloadedSongsManager: Error processing song ${songId}:`,
            songError
          );
        }
      }
      ...
      console.error(
        'DownloadedSongsManager: Error loading downloaded songs:',
        error
      );
      setDownloadedSongs([]);
      setDownloadedSongsMetadata({});
  ...
  const isSongDownloaded = useCallback(
    async (songId) => {
      try {
        ...
        console.error(
          `DownloadedSongsManager: Error checking download status for ${songId}:`,
          error
        );
        return false;
      }
  ...
  const getDownloadedSongMetadata = useCallback(
    (songId) => {
    ...
    async (songId) => {
      try {
        ...
        try {
          if (onDownloadedSongsChanged) {
            onDownloadedSongsChanged(
              downloadedSongs.filter((song) => song.id !== songId)
            );
          }
          ...
          console.error(
            `DownloadedSongsManager: Error removing song ${songId}:`,
            error
          );
          return false;
        }
        ...
        console.error(
          `DownloadedSongsManager: Error in removeDownloadedSong for ${songId}:`,
          error
        );
        return false;
      }
  ...
  const getDownloadStats = useCallback(() => {
    ...
    const totalSize = downloadedSongs.reduce(
      (sum, song) => sum + (song.fileSize || 0),
      0
    );
    const oldestDownload =
      downloadedSongs.length > 0
        ? Math.min(
            ...downloadedSongs.map((song) => song.downloadTime || Date.now())
          )
        : null;
    const newestDownload =
      downloadedSongs.length > 0
        ? Math.max(
            ...downloadedSongs.map((song) => song.downloadTime || Date.now())
          )
  ...
  const searchDownloadedSongs = useCallback(
    (query) => {
      if (!query || query.trim() === '') {
  ...
  const filterDownloadedSongs = useCallback(
    (criteria) => {
  ...
  useEffect(() => {
    ...
    const deleteListener = DeviceEventEmitter.addListener('songDeleted', () => {
      loadDownloadedSongs();
    });
  ...
  }, [loadDownloadedSongs]); // Remove dependencies to prevent re-initialization
