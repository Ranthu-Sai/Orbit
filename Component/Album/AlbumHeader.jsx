...
const IMAGE_SIZE = SCREEN_WIDTH * 0.4; // 40% of screen width
...
export const AlbumHeader = ({
  ...
  useEffect(() => {
    ...
    ImageColors.getColors(imageUrl, {
      ...
      .catch((err) => {
        console.warn('Failed to extract color for AlbumHeader:', err);
      });
    ...
    const checkPlaybackState = async () => {
      try {
        ...
        const isPlayerPlaying =
          state === TrackPlayer.STATE_PLAYING || state === 3;
  ...
  useEffect(() => {
    ...
    const startListenerId = EventRegister.addEventListener(
    ...
    const completeListenerId = EventRegister.addEventListener(
  ...
  useEffect(() => {
    const checkAllDownloaded = async () => {
      ...
      const StorageManager =
        require('../../Utils/StorageManager').StorageManager;
      ...
      const allDownloadedStatus =
        downloadedCount === songs.length && songs.length > 0;
    ...
    const downloadCompleteId = EventRegister.addEventListener(
      'download-complete',
      checkAllDownloaded
    );
    const batchCompleteId = EventRegister.addEventListener(
      'batch-download-complete',
      checkAllDownloaded
    );
    const downloadRemovedId = EventRegister.addEventListener(
      'download-removed',
      checkAllDownloaded
    );
  ...
  const handleLikePress = useCallback(async () => {
    ...
    try {
      if (isLiked) {
        ...
        setIsLiked(false);
        ToastAndroid.show(
          `Removed "${title || 'Album'}" from favorites`,
          ToastAndroid.SHORT
        );
        ...
        setIsLiked(true);
        ToastAndroid.show(
          `Added "${displayName}" to favorites`,
          ToastAndroid.SHORT
        );
        // Emit event to refresh favorites screen
        DeviceEventEmitter.emit('favorites-updated');
  ...
  const formatSongsForPlayer = useCallback(
    async (shuffle = false) => {
        ...
        <View style={styles.contentSection}>
          ...
          <View style={styles.infoRow}>
            ...
            {totalDuration > 0 && (
              ...
              >
                <Text
                  style={[styles.infoChipText, { color: theme.colors.text }]}
                >
            ...
            {(isHiRes || qualityLabel) && (
              <View
                style={[styles.qualityBadge, { backgroundColor: '#1DB954' }]}
              >
