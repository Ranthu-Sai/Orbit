...
export const useDeviceLibrary = () => {
                ...
                return currentTracks.map(track => {
                    ...
                    if (meta) {
                        ...
                        return {
                            ...
                            year: meta.year,
                            genre: meta.genre,
                            artwork: meta.localArtworkPath ? { uri: meta.localArtworkPath } : track.artwork,
                            cover: meta.localArtworkPath ? { uri: meta.localArtworkPath } : track.cover
                        };
                    }
                    return track;
                });
    ...
    const fetchLocalMusic = useCallback(async (useCache = true) => {
        ...
        try {
            ...
            console.warn(err);
            if (!hasCachedData) setError('Failed to scan music.');
        } finally {
            setLoading(false);
        }
