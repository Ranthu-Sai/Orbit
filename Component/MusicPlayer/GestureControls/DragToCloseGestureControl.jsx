...
export const useDragToCloseGestureControl = (onClose) => {
      ...
      Gesture.Pan()
        ...
        .onFinalize((e) => {
          ...
          const verticalDominance =
            Math.abs(e.velocityY) > Math.abs(e.velocityX) * 1.5;
          ...
          if (
            verticalDominance &&
            (significantDownwardMovement || fastDownwardSwipe)
          ) {
            runOnJS(onClose)();
          }
        })
   ...
   */
  const createCustomDragToCloseGesture = useCallback(
    (threshold = 50, velocityThreshold = 500) => {
      return Gesture.Pan()
        .activeOffsetY([10, 1000])
        .failOffsetX([-15, 15])
        .onBegin((e) => {
  ...
  const analyzeDragGesture = useCallback((gestureEvent) => {
    const verticalDominance =
      Math.abs(gestureEvent.velocityY) > Math.abs(gestureEvent.velocityX) * 1.5;
    const significantDownwardMovement = gestureEvent.translationY > 50;
    const fastDownwardSwipe = gestureEvent.velocityY > 500;

    const isValidDragToClose =
      verticalDominance && (significantDownwardMovement || fastDownwardSwipe);
