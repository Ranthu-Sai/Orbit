...
import { PlayNextSong, PlayPreviousSong, PlaySong, PauseSong } from '../../MusicPlayerFunctions';
...
const SkipButton = memo(({ direction = 'next', size = 36, color = '#fff' }) => {
    ...
    return (
        <ControlButton
            iconName={direction === 'next' ? 'skip-next-circle' : 'skip-previous-circle'}
            size={size}
            color={color}
            onPress={handleSkip}
        />
...
const OptimizedPlaybackControls = memo(({
    ...
    return (
        <View style={styles.container}>
            {/* Previous */}
            <SkipButton direction="previous" size={skipButtonSize} color={iconColor} />
