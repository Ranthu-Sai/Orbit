...
export const SaavnHomeFeed = forwardRef(
  ({ refreshing, onRefreshComplete }, ref) => {
    const [Data, setData] = useState({
      data: { charts: [], playlists: [], new_trending: [] },
    });
    ...
    async function fetchSaavnData(forceRefresh = false) {
      ...
      const cacheKey = generateCacheKey(CACHE_KEYS.HOME, 'saavn_v3');
      ...
      try {
        ...
        if (data && isMounted.current) {
          ...
          if (forceRefresh && fetchedData.data) {
            ...
            if (
              fetchedData.data.new_trending &&
              fetchedData.data.new_trending.length > 0
            ) {
              fetchedData.data.new_trending = shuffleArray(
                fetchedData.data.new_trending
              );
            }
          }
    ...
    const loadMoreSections = () => {
      ...
      if (visibleCount < totalSections) {
        setVisibleCount((prev) =>
          Math.min(prev + SECTIONS_PER_LOAD, totalSections)
        );
      }
    };
    ...
    const sections = [
      { id: 'genres', component: <DisplayTopGenres key="genres" /> },
      {
        ...
        component: (
          <View key="songs-0" style={{ paddingHorizontal: 13 }}>
            <HorizontalScrollSongs id={getChartId(0)} type="chart" />
          </View>
        ),
      },
      {
      ...
      },
      {
        ...
        component: (
          <View key="songs-1" style={{ paddingHorizontal: 13, marginTop: 8 }}>
            <HorizontalScrollSongs id={getChartId(1)} type="chart" />
          </View>
        ),
      },
      {
      ...
      },
      {
        id: 'songs-2',
        component: (
          <PaddingConatiner key="songs-2">
            <HorizontalScrollSongs id={getChartId(2)} type="chart" />
          </PaddingConatiner>
        ),
      },
      {
        ...
        component: (
          <PaddingConatiner key="songs-3">
            <HorizontalScrollSongs id={getChartId(3)} type="chart" />
          </PaddingConatiner>
        ),
      },
    ];
    ...
    return (
      <View>{sections.slice(0, visibleCount).map((s) => s.component)}</View>
    );
  }
);
