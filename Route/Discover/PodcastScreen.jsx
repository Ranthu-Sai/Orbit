...
export const PodcastScreen = () => {
  ...
  const loadMoreTrending = async () => {
    if (loadingMoreTrending || !hasMoreTrending || isSearching) {
      return;
    }
    ...
    try {
      ...
      const catName =
        selectedCategory?.id !== null ? selectedCategory?.name : null;
  ...
  const loadMoreSearch = async () => {
    if (loadingMoreSearch || !hasMoreSearch || !isSearching) {
      return;
    }
  ...
  const handleCategorySelect = useCallback(async (category) => {
    ...
    try {
      ...
      const response = await getTrendingPodcasts(
        ITEMS_PER_PAGE * 2,
        null,
        catName
      );
