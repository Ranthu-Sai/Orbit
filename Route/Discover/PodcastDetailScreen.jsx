...
import {
  ...
  EpisodeListSkeleton,
} from '../../Component/Podcast/PodcastDetailSkeleton';
...
export const PodcastDetailScreen = () => {
  ...
  const handlePlayEpisode = async (episode) => {
    ...
    const episodeIndex = episodes.findIndex(
      (ep) => ep.id === episode.id || ep.guid === episode.guid
    );
    ...
    try {
      ...
      const tracksFromEpisode = episodes
        .slice(episodeIndex)
        .map(episodeToTrack);
      ...
      <FlatList
        ...
        ListHeaderComponent={() => (
          <>
            ...
            {/* Loading episodes */}
            {loadingEpisodes && <EpisodeListSkeleton />}
          </>
        )}
        renderItem={({ item }) => (
          <EpisodeCard episode={item} onPlay={handlePlayEpisode} />
        )}
        ListEmptyComponent={() =>
          !loadingEpisodes && (
            <Text
