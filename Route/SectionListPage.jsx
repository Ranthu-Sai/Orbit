...
const SectionListPage = () => {
  ...
  const loadData = async () => {
    ...
    try {
      setLoading(true);
      const data = await YouTubeMusicService.getSection(
        endpoint.browseId,
        endpoint.params
      );
  ...
  const handlePress = useCallback(
    (item) => {
      ...
      if (itemType === 'song' || itemType === 'video') {
        ...
        const listToPlay = items
          .filter((i) => getType(i) === 'song' || getType(i) === 'video')
          .map((s) => ({
            ...s,
            url: s.videoId || s.id,
            artwork: s.thumbnail || s.thumbnails?.[0]?.url,
            artist:
              s.artist || s.artists?.map((a) => a.name).join(', ') || 'Unknown',
          }));

        // Find index in the playlist
        const index = listToPlay.findIndex(
          (s) => s.videoId === item.videoId || s.id === item.id
        );
  ...
  const isGrid =
    items.length > 0 &&
    getType(items[0]) !== 'song' &&
    getType(items[0]) !== 'video';
  ...
  return (
    ...
    >
      ...
      {loading && items.length === 0 ? (
        initialType === 'song' ||
        initialType === 'video' ||
        (endpoint?.params && endpoint.params.includes('songs')) ? (
