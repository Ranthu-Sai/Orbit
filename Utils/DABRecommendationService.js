...
class DABRecommendationService {
  ...
  async loadTopPlayedSongs() {
    try {
      ...
      console.error(
        'DABRecommendationService: Failed to load top played',
        error
      );
    }
  }
  ...
  async getRecommendations(count = 10) {
    ...
    for (const seed of seeds) {
      try {
        const similar = await lastFMService.getSimilarTracks(
          seed.artist,
          seed.track,
          30
        );
        ...
        console.error(
          `🧠 DABRecs: Failed to get similar for ${seed.track}`,
          error
        );
      }
    }
    ...
    for (const rec of toResolve) {
      ...
      try {
        ...
        if (result && result.song) {
          ...
          if (this.recentHistory.has(songId)) {
            continue;
          }
    ...
    const _uniqueArtists = Object.keys(artistCounts).length;
    const _uniqueAlbums = Object.keys(albumCounts).length;
    return resolved;
  }
