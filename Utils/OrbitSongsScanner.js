...
class OrbitSongsScanner {
   ...
   */
  static async scanOrbitSongs(_useCache = true) {
    ...
    try {
      ...
      if (!songsDir) {
        ...
        ToastAndroid.show(
          '❌ Could not determine songs directory',
          ToastAndroid.LONG
        );
        return [];
      }
      ...
      if (!dirExists) {
        ...
        try {
          ...
          console.error(
            '❌ [OrbitScanner] Failed to create directory:',
            mkdirError
          );
          ToastAndroid.show(
            `Failed to create dir: ${mkdirError.message}`,
            ToastAndroid.LONG
          );
        }
        return [];
      ...
      ToastAndroid.show(
        `Found ${files.length} total items`,
        ToastAndroid.SHORT
      );
      ...
      const audioFiles = files.filter((file) => {
        ...
        const isAudio =
          this.SUPPORTED_EXTENSIONS.includes(ext) && file.isFile();
        if (isAudio) {
        }
        return isAudio;
      });
      ...
      ToastAndroid.show(
        `Extracting metadata from ${filePaths.length} files...`,
        ToastAndroid.SHORT
      );
      ...
      try {
        ...
        ToastAndroid.show(
          `✅ Extracted ${metadataArray.length} metadata`,
          ToastAndroid.SHORT
        );
        ...
        console.error(
          '❌ [OrbitScanner] Failed to extract metadata:',
          metadataError
        );
        console.error(
          '❌ [OrbitScanner] Error details:',
      ...
      ToastAndroid.show(
        `✅ Successfully scanned ${songs.length} songs`,
        ToastAndroid.LONG
      );
      return songs;
    } catch (error) {
