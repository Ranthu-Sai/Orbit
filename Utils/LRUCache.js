...
class LRUCacheManager {
  ...
  async _ensureMetadataLoaded() {
    ...
    try {
      ...
      console.warn(
        `[LRUCache:${this.namespace}] Failed to load metadata:`,
        error.message
      );
  ...
  async _saveMetadata() {
    try {
      ...
      console.warn(
        `[LRUCache:${this.namespace}] Failed to save metadata:`,
        error.message
      );
    }
  }
  ...
  async reconcile() {
    try {
      ...
      console.warn(
        `[LRUCache:${this.namespace}] Reconciliation failed:`,
        error.message
      );
      return 0;
    }
  ...
  async evictOldest(count = null) {
    ...
    try {
      ...
      console.error(
        `[LRUCache:${this.namespace}] Eviction failed:`,
        error.message
      );
      return 0;
    } finally {
  ...
  async get(key) {
    try {
      ...
      console.warn(
        `[LRUCache:${this.namespace}] Get failed for ${key}:`,
        error.message
      );
      return null;
    }
  ...
  async set(key, data, expiration = null) {
    try {
      ...
      try {
        ...
        if (this.metadata.size > this.config.maxEntries) {
          ...
          this.evictOldest().catch((e) =>
            console.warn(
              `[LRUCache:${this.namespace}] Background eviction failed:`,
              e.message
            )
          );
        }
        ...
        if (this._isDiskFullError(storageError)) {
          const now = Date.now();
          const shouldWarn =
            now - this.lastDiskFullWarning > this.diskFullWarningCooldown;
          ...
          if (evicted > 0) {
            ...
            try {
              ...
              if (shouldWarn) {
                console.warn(
                  `[LRUCache:${this.namespace}] Save failed even after emergency eviction`
                );
              }
            }
          } else if (shouldWarn) {
            console.warn(
              `[LRUCache:${this.namespace}] Emergency eviction could not find any items to remove`
            );
          }
        }
      ...
      console.warn(
        `[LRUCache:${this.namespace}] Set failed for ${key}:`,
        error.message
      );
      return false;
    }
  ...
  async remove(key) {
    try {
      ...
      console.warn(
        `[LRUCache:${this.namespace}] Remove failed for ${key}:`,
        error.message
      );
    }
  }
  ...
  async clear() {
    try {
      ...
      console.error(
        `[LRUCache:${this.namespace}] Clear failed:`,
        error.message
      );
    }
  }
...
const chartCache = new LRUCacheManager({
  namespace: 'chart',
  maxEntries: 20,
  evictionPercent: 0.2,
});

export { LRUCacheManager, playlistCache, albumCache, apiCache, chartCache };
