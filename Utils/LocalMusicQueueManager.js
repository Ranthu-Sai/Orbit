...
class LocalMusicQueueManager {
    ...
    async _loadNextBatch() {
        if (this.isLoading || !this.isActive || this.loadedEndIndex >= this.allSongs.length) {
            return;
        }
        ...
        try {
            ...
            const batchEnd = Math.min(batchStart + BACKGROUND_BATCH_SIZE, this.allSongs.length);
            ...
            console.error('❌ LocalMusicQueueManager: Error loading batch:', error.message);
        } finally {
    ...
    async _cleanupOldTracks(currentIndex) {
        ...
        try {
            ...
            console.error('❌ LocalMusicQueueManager: Error cleaning up:', error.message);
        }
    }
    ...
    getStatus() {
        return {
            ...
            progress: this.allSongs.length > 0 ? this.loadedEndIndex / this.allSongs.length : 0
        };
    }
