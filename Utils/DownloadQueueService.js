...
class DownloadQueueService {
  ...
  static async handleQueueEnded() {
    try {
      ...
      if (
        !lastTrack?.isDownloaded &&
        !lastTrack?.isLocal &&
        lastTrack?.sourceType !== 'downloaded' &&
        lastTrack?.sourceType !== 'mymusic'
      ) {
        return;
      }
  ...
  static async suggestSongsFromSaavn(lastTrack) {
    try {
      ...
      const searchQuery =
        lastTrack.artist || lastTrack.title || 'popular hindi songs';
      ...
      for (const song of searchResult.data.results) {
        ...
        const songUrl =
          song.downloadUrl[quality]?.url || song.downloadUrl[0]?.url;
        if (!songUrl) {
          continue;
        }
      ...
      ToastAndroid.show(
        `Playing similar songs by ${lastTrack.artist || 'various artists'}`,
        ToastAndroid.SHORT
      );
    } catch (error) {
      console.error(
        '[DownloadQueueService] Error fetching suggestions:',
        error
      );
