...
class LastFMService {
    ...
    try {
      ...
      if (data && data.user) {
        return {
          ...
          avatarUrl:
            data.user.image?.[2]?.['#text'] ||
            data.user.image?.[1]?.['#text'] ||
            null,
  ...
  async _apiRequest(method, params = {}, requiresAuth = false) {
    if (!this.isInitialized) {
      throw new Error(
        'LastFMService not initialized. Call initialize() first.'
      );
    }
    ...
    const response = await fetch(LASTFM_API_BASE, {
      ...
      headers: {
        ...
        'User-Agent':
          'Orbit Music Player (https://github.com/gauravxdev/Orbit)',
      },
  ...
  _cleanTrackTitle(title) {
    ...
    return (
      title
        ...
        .replace(
          /[\(\[](remaster|remix|version|edit|radio)[^\)\]]*[\)\]]/gi,
          ''
        )
  ...
  async getSimilarTracks(artist, track, limit = 20) {
    try {
      ...
      try {
        ...
        console.error(
          '🧠 LastFM Brain: Mood fallback also failed',
          fallbackError
        );
        return [];
      }
  ...
  async getMoodBasedRecommendations(artist, track, limit = 20) {
    ...
    try {
      ...
      if (moodTags.length > 0) {
        ...
        for (const tagTracks of tagResults) {
          for (const t of tagTracks) {
            ...
            if (
              !seenTracks.has(key) &&
              !seenArtists.has(t.artist.toLowerCase())
            ) {
              seenTracks.add(key);
              ...
              const artistCount = results.filter(
                (r) => r.artist.toLowerCase() === artistLower
              ).length;
              if (artistCount >= 2) {
      ...
      if (similarArtists.length > 0 && results.length < limit) {
        const artistPromises = similarArtists
          .slice(0, 3)
          .map((a) => this.getArtistTopTracks(a.name, 5));
      ...
      console.error(
        '🧠 LastFM Brain: getMoodBasedRecommendations failed',
        error
      );
      return [];
    }
  ...
  async getArtistTopTracks(artist, limit = 20) {
    try {
      ...
      const results = tracks.map((t) => ({
        ...
        track: t.name || '',
        match:
          maxPlaycount > 0 ? (parseInt(t.playcount, 10) || 0) / maxPlaycount : 0.5,
        url: t.url || '',
      }));
