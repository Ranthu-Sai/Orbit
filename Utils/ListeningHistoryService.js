...
class ListeningHistoryService {
  ...
  async handleTrackChange(event) {
    ...
    this.saveSession().catch((e) =>
      console.warn('ListeningHistory: Save session failed:', e)
    );

    // Check if we should refresh the home feed
    if (
      this.sessionPlayCount >= SONGS_UNTIL_REFRESH &&
      this.sessionPlayCount % SONGS_UNTIL_REFRESH === 0
    ) {
      // PERFORMANCE: Non-blocking - run in background
      this.markHomeFeedForRefresh().catch((e) =>
        console.warn('ListeningHistory: markHomeFeedForRefresh failed:', e)
      );
    }
  }
  ...
  getStats() {
    return {
      ...
      songsUntilRefresh:
        SONGS_UNTIL_REFRESH - (this.sessionPlayCount % SONGS_UNTIL_REFRESH),
      lastSongId: this.lastPlayedSongId,
    };
  }
