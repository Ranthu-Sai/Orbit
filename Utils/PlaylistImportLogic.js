...
export const importFromLink = async (url, onProgress) => {
  try {
    ...
    if (!linkInfo) {
      throw new Error(
        'Unsupported or invalid link. Please use a valid Spotify or YouTube link.'
      );
    }
  ...
  try {
    ...
    if (type === 'album') {
      ...
      if (albumInfo) {
        ...
        let albumArtist =
          albumInfo.artist || albumInfo.owner || albumInfo.author;
        ...
        if (albumName) {
          ...
          try {
            ...
            const albumMatch =
              searchResults?.find(
                (item) => item.title?.toLowerCase() === albumName.toLowerCase()
              ) || searchResults?.[0];
          ...
          console.warn(
            'Album info returned without name/title, falling back to songs'
          );
    ...
    for (const song of songsToImport) {
      if (song) {
        ...
        if (addedCount % 5 === 0) {
          onProgress(
            addedCount,
            songsToImport.length,
            `Saved ${addedCount} songs...`
          );
        }
      }
    }
...
const handleSpotifyImport = async (type, id, onProgress) => {
  ...
  const matchedSongs = await matchSpotifyTracksOnYouTube(
    tracksToMatch,
    onProgress
  );
...
const matchSpotifyTracksOnYouTube = async (tracks, onProgress) => {
  ...
  for (let i = 0; i < tracks.length; i += CHUNK_SIZE) {
    ...
    onProgress(
      processed,
      tracks.length,
      `Matching: ${processed}/${tracks.length}`
    );
  }
...
const handleYouTubeImport = async (type, id, onProgress) => {
  ...
  if (type === 'playlist') {
    ...
    if (!tracks?.length) {
      throw new Error('Playlist empty/not found');
    }
    name = playlist.title;
    ...
    if (!tracks?.length) {
      throw new Error('Album empty/not found');
    }
    name = album.title;
  ...
  if (!songs.length) {
    throw new Error('No songs to import');
  }
 ...
 */
const fetchYouTubeSongs = async (type, id, _onProgress) => {
  let songs = [];
